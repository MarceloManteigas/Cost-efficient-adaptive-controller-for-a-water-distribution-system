# -*- coding: utf-8 -*-
"""
This files will be used to simulate the real network, based on the hidraulic
model called benchmark2018
...
def RealNetwork(x,iChart,random):
    nInc = len(x)
    ...
    def Caudal_VC(ti, tf,random,inc):
    ...
    for i in range(0, nInc):
        ...
        QVC = Caudal_VC(timeInc[i]['startTime'], timeInc[i]['endTime'],random,i)
        QR = Caudal_R(timeInc[i]['startTime'], timeInc[i]['endTime'],random,i); QRmed= QR/timeInc[i]['duration']
        #
    ...
    return fObjRest, consumo,hFinal;
...
fObjRest, Sensibil,consumo1=RealNetwork(x,1)


plt.plot(list(range(0,nInc)),consumo1,list(range(0,nInc)),consumo2)
plt.show()
'''
#a,b=controllerSimulation(xopt,yopt,nInc)
    #----------------------
#nInc = 24 #96 #24
# Declaração de solução
#x = [0.5 for i in range (0, nInc)]
#fObjRest, Sensibil,y = RealNetwork(x,1,nInc)
